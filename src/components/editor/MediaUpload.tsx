...
import { ImageIcon, Upload, X, LogIn } from 'lucide-react'
import { cn, compressImage } from '@/lib/utils'
import { toast } from 'sonner'
import { LockOverlay } from '../common/LockOverlay'

export const MediaUpload = () => {
    const { templateId, mainImage, setMainImage, logo, setLogo, userTier } = useStore()
    const imageInputRef = useRef<HTMLInputElement>(null)
    const logoInputRef = useRef<HTMLInputElement>(null)

    if (templateId === 'PublicNotice' || templateId === 'ViralQuote') return null
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, type: 'image' | 'logo') => {
        ...
        reader.onload = async (event) => {
            ...
            if (type === 'image') setMainImage(compressed)
